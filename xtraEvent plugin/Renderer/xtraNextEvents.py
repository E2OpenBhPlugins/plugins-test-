# -*- coding: utf-8 -*-
# by digiteng...05.2020, 08.2020, 11.2021
# for channellist,
...
class xtraNextEvents(Renderer):
	...
	def showPicture(self):
		...
		try:
			...
			if ref:
				events = self.epgcache.lookupEvent(['IBDCT', (ref.toString(), 0, 1, -1)]) # max 10
				# events = self.epgcache.lookupEvent(['IBDCTT', (ref.toString(), 0, -1, -1)])
				if events:
					evnt = events[self.nxEvnt][4]
					evntNm = REGEX.sub('', evnt).strip()
					pstrNm = pathLoc + "xtraEvent/{}/{}.jpg".format(self.nxEvntUsed, evntNm)
