...
from enigma import ePoint, eWidget, eLabel, eSize, gFont, ePixmap, loadJPG, eEPGCache, addFont, eTimer
...
class xtraBackdropList(Renderer):
	...
	def showImages(self):
		...
		px, py = self.szX // 3, self.szY - (self.szY // 3)
		nx, ny = px, self.szY // 3
		font_size = self.szY // 11
		if self.mode == "trio":
			if self.epgcache is not None and events:
				try:
					...
					evntNm1 = "%02d:%02d - %s\n"%(bt[3], bt[4], evnt)
					pstrNm = pathLoc + "xtraEvent/backdrop/{}.jpg".format(evntNm)
					...
					self.eventName1.move(ePoint(0, self.szY-(self.szY//3)+5))
					self.eventName1.setFont(gFont("Regular", font_size))
				...
				try:
					...
					self.eventName2.move(ePoint(px + 5, self.szY-(self.szY//3)+5))
					self.eventName2.setFont(gFont("Regular", font_size))
				...
				try:
					...
					self.eventName3.move(ePoint(((px)*2) + 10, self.szY-(self.szY//3)+5))
					self.eventName3.setFont(gFont("Regular", font_size))
