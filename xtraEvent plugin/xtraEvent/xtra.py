...
from enigma import eTimer, eLabel, eServiceCenter, eServiceReference, ePixmap, eSize, ePoint, loadJPG, loadPNG, iServiceInformation, eEPGCache, getBestPlayableServiceReference, getDesktop
...
config.plugins.xtraEvent.searchMOD = ConfigSelection(default = lng.get(lang, '14'), choices = [(lng.get(lang, '13')), (lng.get(lang, '14'))])
...
config.plugins.xtraEvent.searchLang = ConfigSelection(default = lng.get(lang, '12'), choices = [((""), lng.get(lang, '12')), ((lang), "stb")])
...
class xtra(Screen, ConfigListScreen):
	def __init__(self, session):
		...
		self['key_yellow'] = Label(_('NO Update'))
...
# CONFIG_________________________________________________________________________________________________________________
		list.append(getConfigListEntry("  " + lng.get(lang, '0'), config.plugins.xtraEvent.onoff, _(lng.get(lang, '0'))))
		if config.plugins.xtraEvent.onoff.value:
			list.append(getConfigListEntry("  " + lng.get(lang, '1'), config.plugins.xtraEvent.cnfg, _(lng.get(lang, '2'))))
			list.append(getConfigListEntry("—"*100))
			if config.plugins.xtraEvent.cnfg.value:
				list.append(getConfigListEntry(lng.get(lang, '3'), config.plugins.xtraEvent.loc, _(lng.get(lang, '4'))))
				list.append(getConfigListEntry(lng.get(lang, '5'), config.plugins.xtraEvent.skinSelect, _(lng.get(lang, '19'))))
				if config.plugins.xtraEvent.skinSelect.value == 'skin_1':
					list.append(getConfigListEntry("   "+lng.get(lang, '69'), config.plugins.xtraEvent.skinSelectColor, _(lng.get(lang, '19'))))
				...
				if config.plugins.xtraEvent.opt_Images.value:
					list.append(getConfigListEntry("\t" + lng.get(lang, '7'), config.plugins.xtraEvent.cnfgSel, _(lng.get(lang, '21'))))
				...
				if config.plugins.xtraEvent.timerMod.value == True:
					list.append(getConfigListEntry(lng.get(lang, '46'), config.plugins.xtraEvent.timer, _(lng.get(lang, '67')),))
				list.append(getConfigListEntry("—"*100))
			list.append(getConfigListEntry("  " + lng.get(lang, '28')))
	...
	def menuS(self):
		if config.plugins.xtraEvent.onoff.value:
			list = [(_(lng.get(lang, '50')), self.brokenImageRemove), (_(lng.get(lang, '73')), self.removeImagesAll), (_("Update"), self.update), (_(lng.get(lang, '35')), self.exit)]
	...
	def update(self):
		try:
			url = requests.get("https://api.github.com/repos/digiteng/xtra/releases")
			new_version = url.json()["name"]
			if version != new_version:
				up_msg = "Current version : {}\nNew version : {}\n\nDo you want UPDATE PLUGIN ?..".format(version, new_version)
...
class manuelSearch(Screen, ConfigListScreen):
	def __init__(self, session):
		...
		self.timer = eTimer()
		self.timer.callback.append(self.intCheck)
